...
func init() {
	...
	if config.Storage == "" {
		config.Storage = fmt.Sprintf("/var/lib/%s", BinaryName)
	}
...
func usage() {
	fmt.Printf("Usage: %s [-b 192.0.2.1:80] [-d example.com] [-l /var/log/%s] [-m 0.2] [-s /var/lib/%s] [-t /tmp]\n", BinaryName, BinaryName, BinaryName)
	flag.PrintDefaults()
	fmt.Println("")
}
